...
def make_median_sigma_masked_psc(psc, sigma_thresh=2.5):
    med_image = np.median(psc, axis=0)
    med_rgb_image = get_rgb_data(med_image)
    ...
    for i, color in enumerate('rgb'):
        m0 = np.ma.array(
            med_rgb_image[i],
            mask=~np.logical_or(
                sigma_clip(med_rgb_image[i], sigma=4).mask,
                med_rgb_image[i].mask
            )
        )
        med_masks.append(m0)    
    ...
    full_stamp = (med_masks[0].filled(0) + 
                  med_masks[1].filled(0) + 
                  med_masks[2].filled(0)
                 )
    pix_mask = full_stamp == 0
    
    # Build the complete stamp
    d0 = [np.ma.array(target_psc.reshape(len(target_psc), *size)[i], mask=pix_mask)
          for i in range(len(target_psc))
         ]
