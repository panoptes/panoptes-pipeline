...
def test_an_inconsistent_camera_serial_is_flagged(processed_tree):
    """Two such cases exist in the archive and both are data defects."""
    processed = processed_tree(count=3)
    paths = list(index.find_documents(processed))
    document = products.read_document(paths[0])
...
def test_build_writes_both_files(processed_tree, tmp_path):
    written = index.build(processed_tree(count=2), tmp_path / "index")

    assert set(written) == {"frames", "observations"}
